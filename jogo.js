var timerId = null // variável que armazena a chamada da função timeout
 ...
 function iniciaJogo(){
 	...
 	//imprimir qtde de balôes inteiros
 ...
 function contagem_tempo(segundos){
 	...
 	if(segundos == -1){
 		clearTimeout(timerId); // para a função da função do setTimeout
 		game_over();
 		return false;
 	}
...
function situacao_jogo(baloes_inteiros){
	if(baloes_inteiros ==0){
		alert('Parabéns, você conseguiu estourar tdos os balões a tempo');
		para_jogo();
	}
}
